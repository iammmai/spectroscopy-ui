...
import LTSSVGFragmentView, { LTSStateViewData, LTSTransitionViewData, stateCircleSize, XY } from "./LTSSVGFragmentView";
import React from "react";
import { mutatingFilter, mutatingRemove } from "../../../util/arrayMutations";
import {forceX, forceY, forceSimulation, forceCollide, forceManyBody, forceLink} from "d3-force";
import { concat, mapObjectToArray, callOrIgnore, mapObject } from "../../../util/functional";
...
const isTransitionNode = (node: LTSStateViewData | LTSTransitionViewData): node is LTSTransitionViewData => "transitionIndex" in node;
 ...
 */
class LTSInteractiveView extends React.Component<LTSInteractiveViewProps, Record<string, never>> implements LTSInteractiveViewAPI {
    expandAllSingleStep = (): void => {
        Object.keys(this.states).filter((stateKey) => !this.getExpanded(stateKey)).forEach(this.expandState);
    }
    
    collapseAll = (): void => {
        Object.keys(this.states).filter(this.getExpanded).forEach((s) => this.collapseState(s));
    }
    ...
    private getLinkDistance = (link: ForceLink): number => {
        ...
        if (isWeak && this.props.shortWeakSteps) {
            ...
            return this.props.scale * Math.min(Math.max(60 - 2 * Object.keys(this.states).length, 30), 60);
        }
    };
    
    private getNodeCharge = (node: LTSStateViewData | LTSTransitionViewData): number => {
        if (isTransitionNode(node)) {
            return this.props.scale * (node.weak ? -20 : -48);
        } else {
            return this.props.scale * Math.min(Math.max(-48 + 1.2 * Object.keys(this.states).length, -48), -20);
        }
    };
    ...
    private linkForce = forceLink<ForceNode, ForceLink>().distance(this.getLinkDistance).strength(0.2);
    ...
    private generatePath = (targetStateKey: string): string[] | false => {
        ...
        while (current !== this.props.lts.initialState) {
            ...
            if (!previousState || !this.transitions.some((t) => t.source === previousKey && t.target === current)) return this.searchPath(targetStateKey); // full search required because the backwards chain points nowhere or along a transition that was removed when the LTS changed
            current = previousKey;
        ...
        while (frontier.length > 0) {
            ...
            todo.forEach((stateKey) => {
                ...
                this.transitions.forEach((t) => {
                    if (t.target === stateKey && !(t.source in shortestPathForwardLinks)) {
    ...
    private expandState = (stateKey: string): void => {
        if (this.getExpanded(stateKey)) throw new Error("State is already expanded.");
        ...
        if (ltsState.error) {
            callOrIgnore(this.props.onErrorStateExpansionAttempt, stateKey, ltsState.error);
            return;
        }
        ...
        transitions.forEach((ltsTransition, transitionIndex) => {
            ...
            if (this.transitions.some((t) => t.source === stateKey && t.transitionIndex === transitionIndex)) return; // transition is already visible, e.g. due to directed exploration

            const targetStateKey = ltsTransition.target;

            const visualTransition = {
                weak: ltsTransition.weak,
                label: ltsTransition.weak ? (ltsTransition.label ? ltsTransition.label : "τ") : ltsTransition.label,
                detailsLabel: ltsTransition.detailsLabel ? ltsTransition.detailsLabel : undefined,
                source: stateKey,
            ...
            if (targetLtsState.transitions && targetLtsState.transitions.length === 0) {
     ...
     */
    private collapseState = (stateKey: string, protectedTargets: string[] = []): void => {
        ...
        ltsState.transitions?.forEach((transition, transitionIndex) => {
            ...
            const i = this.transitions.findIndex((t) => t.source === stateKey && t.transitionIndex === transitionIndex);
            if (i >= 0) {
    ...
    private autoCollapseNodes = (lastExpandedStateKey: string): void => {
        ...
        if (!path) throw new Error(`Failed to find a path to the initial state while auto-collapsing state ${lastExpandedStateKey}`);
    ...
    private fullUpdate = (): void => {
        ...
        mutatingFilter(this.transitions, (transition) => !!(transition.source in this.states && transition.target in this.states));

        // similarly, remove links that are no longer valid
        mutatingFilter(this.forceLinks, (link) => this.transitions.includes(link.transition));
        ...
        if (this.props.lts) {
            ...
            if (!this.states[this.props.lts.initialState]) this.states[this.props.lts.initialState] = {
                x: 0,
                y: 0,
                initial: true,
                // remaining properties may be incorrect, but will be updated soon anyway
            ...
            for (const stateKey in this.states) {
                ...
                visualState.expandable = !!ltsState.transitions && ltsState.transitions.length > 0 && !this.getExpanded(stateKey);
                visualState.terminal = !!ltsState.transitions && ltsState.transitions.length === 0;
            }
        }

        this.force.nodes(concat<ForceNode>(
        ...
        if (JSON.stringify(newStatus) !== JSON.stringify(this.lastReportedExpansionState)) {
     ...
     */
    exportViewData = (): string => {
        const allThings = concat<ForceNode>(Object.values(this.states), this.transitions);
        const xShift = minBorderDistance(this.props.scale) - allThings.map((n) => n.x).reduce((a, b) => Math.min(a, b));
        const yShift = minBorderDistance(this.props.scale) - allThings.map((n) => n.y).reduce((a, b) => Math.min(a, b));

        const shift = <T extends XY>(n: T): T => ({ ...n, x: n.x + xShift, y: n.y + yShift  });
    ...
    componentDidMount = (): void => {
        this.force.on('tick', () => {
            const limitNodesByBorder = (nodes: ForceNode[]): void => {
                nodes.forEach((n) => {
                    n.x = Math.min(Math.max(n.x, minBorderDistance(this.props.scale)), this.props.width - minBorderDistance(this.props.scale));
                    n.y = Math.min(Math.max(n.y, minBorderDistance(this.props.scale)), this.props.height - minBorderDistance(this.props.scale));
                });
            };
